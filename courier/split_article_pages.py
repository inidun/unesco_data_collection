...
from courier.extract_articles import read_double_pages_from_file
...
for row in df_overlap.to_dict(orient="records"):
    ...
    articles_on_page = article_index[(article_index.courier_id == row["courier_id"]) & article_index.pages.apply(lambda x: row["page"] in x)]
    ...
    page_filenames = glob.glob(os.path.join("../data/courier/pdfbox_output", page_filename_pattern))
